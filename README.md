...
# iProov Android Biometrics SDK v8.2.0-beta
...
3. Add the SDK version to the `dependencies` section in your `build.gradle` file:
    ...
    ```groovy
    dependencies {
        implementation('com.iproov.sdk:iproov:8.2.0-beta')
    }
    ```
...
Both of these methods are demonstrated in the included [Example App](https://github.com/iProov/android/tree/beta/example-app).
...
All strings are prefixed with `iproov__` and you can override them in `strings.xml` (download a copy from [GitHub](https://github.com/iProov/android/blob/beta/resources/strings.xml)).
...
For a simple iProov experience that is ready to run out-of-the-box, see the example project on [GitHub](https://github.com/iProov/android/tree/beta/example-app):

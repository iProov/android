...
# iProov Face SDK for Android v11.1.1
...
* **maven** - a Maven repository for the SDK
...
This guide describes how to integrate the SDK into your Android app, or SDK.
...
The SDK can be downloaded and installed directly into your Android application, or incorporated into your own SDK.
...
- Kotlin 1.8.10 or above
- JVM Target of 18 or above
...
The SDK is provided in Android Library Project (AAR) format as a Maven dependency.
...
3. Add the SDK version to the `dependencies` section in your `build.gradle` file:
    ...
    ```groovy
    dependencies {
        implementation('com.iproov.sdk:iproov:11.1.1')
    }
    ```
...
> **Note**: If the Application context is used, the iProov Activity is launched using the flag _FLAG_ACTIVITY_NEW_TASK_, which will launch iProov in a new Task with the Application's default _taskAffinity_. If the Activity's context is used, the iProov Activity will launch in the same task as the calling Activity. Check the official documentation on [Tasks and the back stack](https://developer.android.com/guide/components/activities/tasks-and-back-stack) for more information.
...
| `certificates`           | Certificates to be passed as a string (the certificate's Subject Public Key Info as SHA-256 hash).<br /> See [below](#certificate-pinning) for more information                                               | iProov Server Certificates                  |
...
Certificate pinning is disabled by default. It is not recommended for most use-cases.

If you are using your own reverse-proxy, you will need to use this option to pin to your own certificates.
...
```kotlin
options.certificates = listOf(Certificate("O8qZKEXWWkMPISIpvB7DUow++JzIW2g+k9z3U/l5V94="), Certificate("Sc7wL4FfOw8ued5w3JOSzu5MzB471PfqdyN4hnMYbX4="))
```
...
Please note that this approach will require you to maintain and update the certificate list for your app to ensure ongoing security and functionality of the SDK
...
You can customize the strings in your app or localize them into a different language in the [standard Android way](https://developer.android.com/training/basics/supporting-devices/languages).

All strings are prefixed with `iproov__` and you can override them in your `strings.xml` (download a copy from [GitHub](https://github.com/iProov/android/blob/master/resources/strings.xml)).
...
- The failure results in a `FailureResult`, which contains a property named `reasons` that is a list of `FailureReason`. A `FailureReason` is an enum which has the following properties:
...
| `BACKGROUND_ISSUE`    | Move to a different location with a neutral background. | ⚠️ | ✅ |
| `DEVICE_ISSUE`        | Try a different device. | ⚠️ | ✅ |
| `EYEWEAR`             | Remove your eyewear. | ⚠️ | ✅ |
| `FACE_NOT_FOUND`      | Align your face in the oval and then try to keep still. | ⚠️ | ✅ |
| `FRAMES_BLURRY`       | Align your face in the oval and then try to keep still. Please try again. | ⚠️ | ✅ |
| `MOTION_ISSUE`        | Try a different device. | ⚠️ | ✅ |
| `LIGHTING_ISSUES`     | Make sure your face is well lit and free from glare. | ⚠️ | ✅ |
| `REJECTED`            | Transaction could not be completed. | ⚠️ | ✅ |
| `SYSTEM_ERROR`        | System error. | ⚠️ | ✅ |
| `TIMEOUT`             | System timeout. | ⚠️ | ✅ |
| `USER_NOT_FOUND`      | Transaction could not be completed. | ⚠️ | ✅ |
| `DEVICE_RESTART`      | Please restart your device and try again. | ⚠️ | ✅ |
| `PROCESSING_FAULT`    | Please try again. | ⚠️ | ✅ |
...
| `CameraException`                 | An error occurred acquiring or using the camera. Applicable when using the external camera support.                                                                                        |
...
They strongly recommend using the lite version for mobile applications, thus the standard version of the SDK has the lite variant as a dependency.
...
To use that just add `"-pf"` to the end of the SDK version name e.g. `10.1.0` -> `10.1.0-pf`.
